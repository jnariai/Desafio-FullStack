...
import { columns } from '@/components/desenvolvedor/columns.ts'
import CreateDesenvolvedor from '@/components/desenvolvedor/CreateDesenvolvedor.vue'

const data = ref<Desenvolvedor[]>([])
...
async function fetchData(): Promise<void> {
  ...
  const response = await fetch(`http://localhost/api/desenvolvedor?${queryParams}`)
...
<template>
  <div class="flex flex-col min-w-full px-5">
    <div class="flex py-5 gap-5">
      <CreateDesenvolvedor @refetch="fetchData" />
      <Input
        v-model="search"
